...
const UNIFORMS = {
  ...
  colorCount: 4,
  scale: 2.0, intensity: 0.54, paramA: 0.47, warp: 0.042,
  detail: 1.536, contrast: 1.158, brightness: 0.0, saturation: 1.0,
  hue: 0.0, vignette: 0.21, blur: 0.002, grain: 0.101,
  seed: 4012.0, rotate: 5.6549, offsetX: 0.11, offsetY: -0.19,
  drift: 0.116, cursorEnabled: false, cursorEffect: 2.0,
  cursorStrength: 0.65, cursorRadius: 0.46, oklab: 0.0, timeScale: -0.727,
};
...
export function ShaderBackground({
  ...
  const activeColors     = colorsProp     ?? UNIFORMS.colors;
  const activeColorCount = colorCountProp ?? UNIFORMS.colorCount;
  const U = UNIFORMS;
  const ts = timeScale ?? U.timeScale;
  ...
  useEffect(() => {
    ...
    // Pad to 8 slots so the shader always receives a full array
    const padded = [...activeColors];
    while (padded.length < 8) padded.push(padded[padded.length - 1] ?? [1, 1, 1]);
    gl.uniform3fv(uni.colors, new Float32Array(padded.flat()));
    gl.uniform4f(uni.shape, scale ?? U.scale, intensity ?? U.intensity, U.paramA, warp ?? U.warp);
    gl.uniform4f(uni.surface, detail ?? U.detail, contrast ?? U.contrast, U.brightness, saturation ?? U.saturation);
    gl.uniform4f(uni.finish, U.hue, vignette ?? U.vignette, blur ?? U.blur, grain ?? U.grain);
    gl.uniform4f(uni.transform, U.seed, rotate ?? U.rotate, drift ?? U.drift, U.oklab);
    gl.uniform4f(uni.cursor, 0, UNIFORMS.cursorEffect, UNIFORMS.cursorStrength, UNIFORMS.cursorRadius);

    let raf = 0, lastNow: number | null = null, disposed = false;
    ...
    const updateLayout = () => {
      bounds = canvas.getBoundingClientRect(); resizeCanvas(); requestRender();
    };
    ...
    function render(now: number) {
      ...
      lastNow = now;
      resizeCanvas();
      gl.uniform4f(uni.scene, canvas.width, canvas.height, ((now - start) / 1000) * ts, activeColorCount);
      gl.uniform4f(uni.space, UNIFORMS.offsetX, UNIFORMS.offsetY, 0, 0);
      gl.uniform4f(uni.cursor, 0, UNIFORMS.cursorEffect, UNIFORMS.cursorStrength, UNIFORMS.cursorRadius);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      if (Math.abs(ts) > 0.0001) requestRender();
      else lastNow = null;
    }
